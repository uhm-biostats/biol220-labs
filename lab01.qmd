...
As with all labs, please complete this lab using *RStudio* running on the UH *Koa Server* (more on what that is below). 
...
Follow the instructions on the ["Setup"](index.qmd) page for setting yourself up on the *Koa Server*.
...
for(i in 1:10) {
  assign(sprintf("koa%s", i), 
         image_trim(image_read(sprintf("images/koa/‎koa.‎%03d.png", i))))
